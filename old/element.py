class element(object):
    ...
    def i_cannot_be(self, value):
        if self.val == value:
            txt = (
                "logical inconsistancy formed, %s.i_cannot_be(%d), but %s.val==%d, previous puzzle state:\n%s"
        ...
        if len(self.possibilities) == 1:
            self._puzzle.log(
                "only one possibility left: %d" % self.possibilities[0], self._loc
            )
            self._puzzle.indent()
    ...
    def i_am(self, value):
        if not (type(value) == type(3) and 0 < value < 10):
            raise TypeError, "i_am() values must be integers, 1-9, received: " + repr(
                value
            )
