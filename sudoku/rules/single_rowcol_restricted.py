        ...
        for attr in ('row', 'col'):
            for boxset in getattr(puzzle, f'box_{attr}s'):
                for box,(o1,o2) in one_and_the_others_iter(boxset):
                    if box.has(v):
                        continue
                    o1 = o1.single_attr_containing_val(v, attr=attr, inc_marks=True, inc_val=True)
                    o2 = o2.single_attr_containing_val(v, attr=attr, inc_marks=True, inc_val=True)

                    if o1 and o2:
                        blessed_no,*broken = set(getattr(x, attr) for x in box) - {o1,o2,}
                        if broken:
                            broken = f'{blessed_no}, {", ".join(broken)}'
                            puzzle.describe_inference(f'this puzzle is broken, we have multiple blessed rows for {v} in {box.short}: {broken}', __name__)
                            return
                        has_no_v = set(e for e in box if not e.value)
                        can_be_v = set(e for e in has_no_v if getattr(e, attr) == blessed_no)
                    ...
                    for no in (o1,o2):
                        cant_be = set(x for x in box if getattr(x, attr) == no and v in x.marks)
                        if cant_be:
                            puzzle.describe_inference(f"{v} can't be in {attr} {no} in {box.short}", __name__)
                            for e in cant_be:
                                e.remove_marks(v)
